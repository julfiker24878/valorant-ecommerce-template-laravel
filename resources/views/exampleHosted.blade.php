...
{{ session([
    ...
    <div class="row mt-5">
        <div class="col-md-8 m-auto">
            <form action="{{ url('/pay') }}" method="POST" class="needs-validation">@csrf
                <ul class="list-group mb-3">
                    ...
                    <li class="list-group-item d-flex justify-content-between">
                        <span>Total (BDT)</span>
                        <input type="hidden" name="total_amount" value="{{ $data['total'] - ( $data['total']*$data['discount'] )/100 + ( $data['payment_method'] ) }}">
                        <strong>{{ $data['total'] - ( $data['total']*$data['discount'] )/100 + ( $data['payment_method'] ) }}</strong>
                    </li>
                </ul>
